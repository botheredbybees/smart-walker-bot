...
ALERTS_SNAPSHOT = [{'type': 'fall', 'timestamp': 1.0}]
INDEX_HTML = '<html><body>dashboard</body></html>'


def _build_response(path):
    return build_response(path, STATUS_SNAPSHOT, MAP_SNAPSHOT, CONVERSATION_SNAPSHOT, ALERTS_SNAPSHOT, INDEX_HTML)


def test_root_path_returns_index_html():
    status, content_type, body = _build_response('/')
    assert status == 200
...
def test_status_path_returns_json_status():
    status, content_type, body = _build_response('/api/status')
    assert status == 200
...
def test_map_path_returns_json_map():
    status, content_type, body = _build_response('/api/map')
    assert status == 200
    assert content_type == 'application/json'
...
def test_conversation_path_returns_json_conversation():
    status, content_type, body = _build_response('/api/conversation')
    assert status == 200
    assert content_type == 'application/json'
    assert json.loads(body) == CONVERSATION_SNAPSHOT


def test_alerts_path_returns_json_alerts():
    status, content_type, body = _build_response('/api/alerts')
    assert status == 200
    assert content_type == 'application/json'
    assert json.loads(body) == ALERTS_SNAPSHOT


def test_unknown_path_returns_404():
    status, content_type, body = _build_response('/nonexistent')
    assert status == 404
